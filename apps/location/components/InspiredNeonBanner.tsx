    ...
    <section className="py-12 bg-white">
      <div className="container mx-auto px-4 max-w-7xl">
        <motion.div 
          ...
          viewport={{ once: true }}
          className="relative w-full bg-brand-dark rounded-[3rem] overflow-hidden flex flex-col md:flex-row items-center justify-between p-12 md:p-20 border border-white/10 group"
        >
          {/* Background Glows */}
          <div className="absolute inset-0 bg-pink-500/5 opacity-0 group-hover:opacity-100 transition-opacity duration-700"></div>
          <div className="absolute top-1/2 left-1/4 -translate-x-1/2 -translate-y-1/2 w-[400px] h-[400px] bg-pink-600/20 blur-[120px] rounded-full"></div>

          {/* Neon Text */}
          <div className="relative z-10 flex-shrink-0 mb-10 md:mb-0">
            <h2 
              className="text-white font-black uppercase tracking-[0.2em] text-5xl md:text-7xl lg:text-8xl leading-none"
              style={{
            ...
            <div className="mt-4 flex space-x-2">
               {[1,2,3].map(i => <div key={i} className="h-1 w-12 bg-pink-500 rounded-full shadow-[0_0_10px_rgba(236,72,153,1)]" />)}
            </div>
          </div>
